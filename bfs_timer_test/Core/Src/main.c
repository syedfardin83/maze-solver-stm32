...
uint16_t dfs_start_time = 0;
uint16_t dfs_end_time = 0;
uint16_t dfs_time = 0;

uint16_t ff_start_time = 0;
uint16_t ff_end_time = 0;
uint16_t ff_time = 0;
...
void bfs_solve(struct Maze* maze, struct Maze* sim_maze);
/* USER CODE END PFP */
...
void bfs_solve(struct Maze* maze, struct Maze* sim_maze){
	for (uint8_t i = 0; i < maze_x_len; i++)
	    ...
	    while (maze->q_r - maze->q_f + 1 > 0)
	    {
	        ...
	        if (maze->bfs_loc[0] == maze->dest[0] && maze->bfs_loc[1] == maze->dest[1])
	        {
	            // build path from current cell to intersection using bfs_parent
	            maze->plen = 0;

	            // start from target (intersection) and walk parents back to start
	            int8_t tx = maze->dest[0];
	            int8_t ty = maze->dest[1];
	            while (!(tx == 0 && ty == 0))
	            {
	                maze->path[maze->plen][0] = tx;
	                maze->path[maze->plen][1] = ty;
...
//	            }
	            // finally add the starting cell
	            maze->path[maze->plen][0] = 0;
	            maze->path[maze->plen][1] = 0;
	            maze->plen++;
